...
function updateNpcs() {
  ...
  for (const npc of npcs) {
    ...
    if (canChase) npc.chasing = true;
    if (!walkMode || distToPlayer > stopChaseDistance) npc.chasing = false;
    ...
    if (npc.moving) {
      const slowWalk = (npc.kind === 'grocery' || npc.kind === 'pharmacy') ? 0.75 : 1.25;
      const speed = npc.chasing ? playerSpeed * 0.82 : slowWalk;
      const nx = dx / len;
      const ny = dy / len;
