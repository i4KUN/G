...
const ADMIN_EMAIL = 'ii4kun@gmail.com';
const CHANGE_COOLDOWN_MS = 60 * 60 * 1000;
const DEATH_RESPAWN_SECONDS = 10;
...
let lastNameChangeAt = 0;
let lastCharacterChangeAt = 0;
let adminRealEmail = '';
let deathCountdownActive = false;
...
function isAdminUser() {
  const email = String(window.auth?.currentUser?.email || currentUserEmail || adminRealEmail || '').toLowerCase();
  const real = String(adminRealEmail || '').toLowerCase();
  return email === ADMIN_EMAIL || real === ADMIN_EMAIL;
}
...
function formatRemaining(ms) {
  ...
  const sec = total % 60;
  return m ? `${m} دقيقة و${sec} ثانية` : `${sec} ثانية`;
}

function canChangeLimited(type, msgElement = null) {
  if (isAdminUser()) return true;
  const last = type === 'character' ? lastCharacterChangeAt : lastNameChangeAt;
  const remain = CHANGE_COOLDOWN_MS - (Date.now() - Number(last || 0));
  if (last && remain > 0) {
    const text = `تستطيع ${type === 'character' ? 'تغيير الشخصية' : 'تغيير الاسم'} بعد ${formatRemaining(remain)}`;
    const msg = msgElement || document.getElementById('settingsMessage');
...
function markLimitedChange(type) {
  if (type === 'character') lastCharacterChangeAt = Date.now();
  else lastNameChangeAt = Date.now();
}
...
function clampZoomValue(value) {
  if (isAdminUser()) {
    const base = walkMode ? WALK_BASE_ZOOM : BUILD_BASE_ZOOM;
    const min = Math.max(0.04, base / Math.pow(ZOOM_STEP, 20));
    const max = base * Math.pow(ZOOM_STEP, 10);
...
function respawnPlayerAfterDeath() {
  const home = getHomeCellKey();
  const cellKey = home || randomSafeCellKey();
  placePlayerAtCell(cellKey, true);
  if (home) centerCameraOnCell(home, walkMode ? WALK_BASE_ZOOM : BUILD_BASE_ZOOM);
  showToast(home ? 'رجعت إلى بيتك' : 'رجعت إلى مكان عشوائي آمن');
}

function startDeathCountdown() {
  if (deathCountdownActive) return;
  deathCountdownActive = true;
  let left = DEATH_RESPAWN_SECONDS;
  const modal = document.getElementById('deathModal');
  const counter = document.getElementById('deathCountdown');
...
const QUEST_TILE_TARGETS = {
  rarePlant: ['All-Pic/pics-map/map-102.png', 'All-Pic/pics-map/map-102-ice.png'],
  well: ['All-Pic/pics-map/map-018.png', 'All-Pic/pics-map/map-018-ice.png', 'All-Pic/pics-map/map-006.png', 'All-Pic/pics-map/map-006-ice.png'],
  palm: ['All-Pic/pics-map/map-074.png', 'All-Pic/pics-map/map-074-ice.png', 'All-Pic/pics-map/map-077.png', 'All-Pic/pics-map/map-077-ice.png', 'All-Pic/pics-map/map-081.png', 'All-Pic/pics-map/map-081-ice.png', 'All-Pic/pics-map/map-083.png', 'All-Pic/pics-map/map-083-ice.png']
};
const STORY_STAGES = [
  { key: 'shepherdCamel', startKind: 'shepherd', targetKind: 'camel', title: 'مرحلة الراعي والجمل' },
  { key: 'archaeology', startKind: 'archaeologist', targetKind: 'ruins', title: 'مرحلة الباحثة الأثرية' },
  { key: 'rarePlants', startKind: 'herbalist', targetKind: 'rarePlant', title: 'مرحلة العطارة والنباتات' },
  { key: 'messageQuest', startKind: 'messenger', targetKind: 'messengerFriend', title: 'مرحلة الرسالة' },
  { key: 'dateQuest', startKind: 'dateWoman', targetKind: 'palm', title: 'مرحلة الضيوف والتمر والنخل' },
  { key: 'fireQuest', startKind: 'fireMan', targetKind: 'well', title: 'مرحلة الخيمة المحترقة' }
];
...
function isQuestMapTileSrc(src, kind = '') {
  const normalized = normalizeSrc(src);
  const list = kind ? QUEST_TILE_TARGETS[kind] : Object.values(QUEST_TILE_TARGETS).flat();
  return list.map(normalizeSrc).includes(normalized);
}

function getQuestKindFromTileSrc(src) {
  for (const [kind, list] of Object.entries(QUEST_TILE_TARGETS)) {
    if (list.map(normalizeSrc).includes(normalizeSrc(src))) return kind;
  }
  return '';
}

function fixedMapTileCenter(item) {
  const rect = editorMapItemRect(item);
  if (!rect) return null;
  return { x: rect.x + rect.w / 2, y: rect.y + rect.h / 2, cell: item.cell || cellFromWorld(rect.x + rect.w / 2, rect.y + rect.h / 2)?.key || '' };
}

function shouldHideFixedMapTileForHome(item) {
  const homeCell = getHomeCellKey();
  if (!homeCell || !item?.cell) return false;
  return item.cell === homeCell && !isQuestMapTileSrc(item.src);
}

function getNearestQuestFixedTarget() {
  if (!walkMode) return null;
  let best = null;
  let bestDist = Infinity;
  for (const item of getFixedEditorMapTiles()) {
    const kind = getQuestKindFromTileSrc(item.src);
    if (!kind) continue;
    const c = fixedMapTileCenter(item);
    if (!c) continue;
    const dist = Math.hypot(player.x - c.x, player.y - c.y);
    if (dist < 110 && dist < bestDist) {
      const cfg = NPC_CONFIG[kind] || {};
      best = { id: 'fixed_' + (item.uid || kind), kind, x: c.x, y: c.y, homeX: c.x, homeY: c.y, dir: 'down', moving: false, fixedMapTarget: true, itemSrc: item.src, label: cfg.label || kind };
      bestDist = dist;
    }
  }
  return best;
...
function getStageIndexByKind(kind) {
  return STORY_STAGES.findIndex(stage => stage.startKind === kind || stage.targetKind === kind);
}

function getFirstIncompleteStage() {
  if (!gameState.quests || typeof gameState.quests !== 'object') gameState.quests = {};
  return STORY_STAGES.find(stage => !gameState.quests[stage.key]?.completed) || null;
}

function isQuestKindAllowed(kind) {
  const idx = getStageIndexByKind(kind);
  if (idx < 0) return true;
  const stage = STORY_STAGES[idx];
  if (gameState.quests?.[stage.key]?.completed) return true;
  const current = getFirstIncompleteStage();
  return !current || current.key === stage.key;
}

function showLockedStageMessage(kind) {
  const idx = getStageIndexByKind(kind);
  const current = getFirstIncompleteStage();
  const needed = current?.title || 'المرحلة الحالية';
  openQuestText('مرحلة مقفلة', `أكمل ${needed} أولًا حتى تفتح هذه المرحلة.`);
}

function findNpcByKind(kind) {
  return npcs.find(n => n.kind === kind && isNpcActive(n)) || npcs.find(n => n.kind === kind) || null;
}

function findFixedTargetByKind(kind) {
  let best = null;
  for (const item of getFixedEditorMapTiles()) {
    if (!isQuestMapTileSrc(item.src, kind)) continue;
    const c = fixedMapTileCenter(item);
    if (!c) continue;
    best = { kind, x: c.x, y: c.y, label: NPC_CONFIG[kind]?.label || kind };
    break;
  }
  return best;
}
...
function targetForKind(kind) {
  if (['rarePlant', 'well', 'palm'].includes(kind)) {
    const fixed = findFixedTargetByKind(kind);
    if (fixed) return fixed;
  }
  const npc = findNpcByKind(kind);
  if (npc) return { kind, x: npc.x, y: npc.y, label: NPC_CONFIG[kind]?.label || kind };
  return findFixedTargetByKind(kind);
}

function getCurrentQuestTarget() {
  const stage = getFirstIncompleteStage();
  if (!stage) return null;
  const q = gameState.quests?.[stage.key] || {};
  let targetKind = stage.startKind;
  if (stage.key === 'shepherdCamel') {
    if (q.foundCamel || findBagItem('camel')) targetKind = 'shepherd';
    else if (q.foundShepherd || q.started) targetKind = 'camel';
  } else if (stage.key === 'archaeology') {
    targetKind = q.foundTarget ? 'archaeologist' : (q.started ? 'ruins' : 'archaeologist');
  } else if (stage.key === 'rarePlants') {
    targetKind = findBagItem('rarePlant') ? 'herbalist' : (q.started ? 'rarePlant' : 'herbalist');
  } else if (stage.key === 'messageQuest') {
    targetKind = findBagItem('message_note') ? 'messengerFriend' : 'messenger';
  } else if (stage.key === 'dateQuest') {
    targetKind = findBagItem('dates') ? 'dateWoman' : (q.started ? 'palm' : 'dateWoman');
  } else if (stage.key === 'fireQuest') {
    targetKind = findBagItem('water_bucket') ? 'fireMan' : (q.started ? 'well' : 'fireMan');
  }
  const target = targetForKind(targetKind);
  return target ? { ...target, stageTitle: stage.title } : null;
}

function drawQuestGuidance() {
  if (!walkMode) { updateQuestArrow(null); return; }
  const target = getCurrentQuestTarget();
  if (!target) { updateQuestArrow(null); return; }
  ...
  ctx.beginPath();
  ctx.arc(p.x, p.y, Math.max(12, 18 * zoom), 0, Math.PI * 2);
  ctx.fill();
  ctx.stroke();
  ctx.restore();
  updateQuestArrow({ x: p.x, y: p.y, label: target.label || 'الهدف' });
}

function updateQuestArrow(target) {
  const arrow = document.getElementById('questArrow');
  const text = document.getElementById('questArrowText');
  if (!arrow) return;
  if (!target || !walkMode) { arrow.classList.add('hidden'); return; }
  const w = canvas.clientWidth;
  const h = canvas.clientHeight;
  const margin = 18;
  const onScreen = target.x >= 0 && target.y >= 0 && target.x <= w && target.y <= h;
  if (onScreen) { arrow.classList.add('hidden'); return; }
  const cx = w / 2;
  const cy = h / 2;
  const dx = target.x - cx;
  const dy = target.y - cy;
  const angle = Math.atan2(dy, dx);
  const x = Math.max(margin, Math.min(w - 120, cx + Math.cos(angle) * (Math.min(w, h) / 2 - 55)));
  const y = Math.max(margin, Math.min(h - 50, cy + Math.sin(angle) * (Math.min(w, h) / 2 - 55)));
  arrow.style.left = `${x}px`;
  arrow.style.top = `${y}px`;
  arrow.querySelector('i')?.style.setProperty('transform', `rotate(${angle + Math.PI / 4}rad)`);
  if (text) text.textContent = target.label || 'الهدف';
  arrow.classList.remove('hidden');
}

function isForbiddenHomeCell(cellKey) {
  if (!cellKey) return true;
  for (const cells of Object.values(NPC_STARTS || {})) {
    if (cells.includes(cellKey)) return true;
  }
  for (const item of getFixedEditorMapTiles()) {
    if (item.cell === cellKey && isQuestMapTileSrc(item.src)) return true;
  }
  return false;
}

function isSafeHomeCell(cellKey) {
  return !!parseCell(cellKey) && !isForbiddenHomeCell(cellKey) && canEditCell(cellKey, false);
}

function randomSafeCellKey() {
  const cells = [];
  for (let row = 1; row <= WORLD_ROWS; row++) {
    for (let col = 1; col <= WORLD_COLS; col++) {
      const key = `${colName(col)}${row}`;
      if (!isForbiddenHomeCell(key)) cells.push(key);
    }
  }
  return cells[Math.floor(Math.random() * cells.length)] || 'K10';
}

function placePlayerAtCell(cellKey, centerCamera = true) {
  const cell = parseCell(cellKey) || parseCell(randomSafeCellKey());
  player.x = (cell.col - 0.5) * CELL;
  player.y = (cell.row - 0.5) * CELL;
  player.dir = 'down';
  lastSafePlayerPosition = null;
  saveLastPlayer();
  if (centerCamera) {
...
function placePlayerAtRandomSafeCell(centerCamera = true) {
  placePlayerAtCell(randomSafeCellKey(), centerCamera);
}
...
function updateNpcs() {
  ...
  for (const npc of npcs) {
    ...
    if (!controller) {
      ...
      if (Number.isFinite(sx) && Number.isFinite(sy)) {
        ...
        if (dist > 0.5) {
          ...
          const nextX = clampNumber(npc.x + dx / dist * step, npcHome.minX, npcHome.maxX, npc.x);
          const nextY = clampNumber(npc.y + dy / dist * step, npcHome.minY, npcHome.maxY, npc.y);
          moveNpcSafely(npc, nextX, nextY);
        }
      }
      continue;
    }
    ...
    if (npc.moving) {
      ...
      const nx = dx / len;
      const ny = dy / len;
      const nextX = clampNumber(npc.x + nx * speed, npcHome.minX, npcHome.maxX, npc.x);
      const nextY = clampNumber(npc.y + ny * speed, npcHome.minY, npcHome.maxY, npc.y);
      moveNpcSafely(npc, nextX, nextY);
...
function getNearestInteractNpc() {
  ...
  const fixedTarget = getNearestQuestFixedTarget();
  if (fixedTarget) {
    const dist = Math.hypot(player.x - fixedTarget.x, player.y - fixedTarget.y);
    if (dist < bestDist) best = fixedTarget;
  }
  return best;
}
...
function openNpcInteraction(npc = lastNearbyNpc) {
  ...
  if (!isQuestKindAllowed(npc.kind)) return showLockedStageMessage(npc.kind);
  if (!npc.fixedMapTarget) {
...
function setNpcModalTitle(title, src = '') {
  const titleBox = document.getElementById('npcTitle');
  if (!titleBox) return;
  const safeTitle = String(title || '').replace(/[<>]/g, '');
  const safeSrc = String(src || '').replace(/"/g, '&quot;');
  titleBox.classList.add('npcTitleWithFace');
  titleBox.innerHTML = safeSrc ? `<img class="npcFace" src="${safeSrc}" alt=""><span>${safeTitle}</span>` : `<span>${safeTitle}</span>`;
}

function currentNpcFaceSrc(kind = '') {
  const k = kind || activeTalkingNpc?.kind || '';
  return NPC_CONFIG[k]?.src || '';
}
...
function openShop(title, shopKey, target) {
  ...
  setNpcModalTitle(title, currentNpcFaceSrc(shopKey));
...
function handleQuestNpc(kind) {
  ...
  if (kind === 'rarePlant') { if (!q.rarePlants?.started) return openQuestText('نبتة نادرة', 'ابدأ المرحلة من العطارة أولًا.'); if (!q.rarePlants?.completed && !findBagItem('rarePlant') && addBagItem({ id:'rarePlant', name:'نبتة نادرة', type:'quest', img:'All-Pic/pics-map/map-102.png' })) showToast('تمت إضافة النبتة إلى الحقيبة'); return openQuestText('نبتة نادرة', findBagItem('rarePlant') ? 'تمت إضافة النبتة إلى الحقيبة. ارجع للعطارة.' : 'تم العثور على نبتة نادرة.'); }
  ...
  if (kind === 'well') { if (!q.fireQuest?.started) return openQuestText('البئر', 'ابدأ المرحلة من صاحب الخيمة أولًا.'); if (!q.fireQuest?.completed && !findBagItem('water_bucket') && addBagItem({ id:'water_bucket', name:'سطل ماء', type:'quest', img:'All-Pic/pics-map/map-018.png' })) showToast('تمت تعبئة الماء وإضافته إلى الحقيبة'); return openQuestText('البئر', 'تمت تعبئة الماء. ارجع إلى صاحب الخيمة.'); }
  ...
  if (kind === 'palm') { if (!q.dateQuest?.started) return openQuestText('النخلة', 'ابدأ المرحلة من صاحبة الضيوف أولًا.'); if (!q.dateQuest?.completed && !findBagItem('dates') && addBagItem({ id:'dates', name:'تمر', type:'quest', img:'All-Pic/pics-map/map-074.png' })) showToast('تمت إضافة التمر إلى الحقيبة'); return openQuestText('النخلة', 'تمت إضافة التمر إلى الحقيبة. ارجع إلى صاحبة البيت.'); }
...
function openQuestText(title, text) {
  const modal = document.getElementById('npcModal');
  setNpcModalTitle(title, currentNpcFaceSrc());
...
function openShepherdQuest(kind) {
  ...
  setNpcModalTitle(kind === 'shepherd' ? 'الراعي' : 'الجمل الضائع', currentNpcFaceSrc(kind));
  listBox.innerHTML = '';
...
function clampCam() {
  ...
  if (isLoggedIn() && !getHomeCellKey() && player) {
    const viewW = canvas.clientWidth / Math.max(zoom, 0.1);
    const viewH = canvas.clientHeight / Math.max(zoom, 0.1);
    const centerX = camX + viewW / 2;
    const centerY = camY + viewH / 2;
    const limit = CELL * 2;
    const safeCenterX = Math.max(player.x - limit, Math.min(player.x + limit, centerX));
    const safeCenterY = Math.max(player.y - limit, Math.min(player.y + limit, centerY));
    camX = Math.max(0, Math.min(maxX, safeCenterX - viewW / 2));
    camY = Math.max(0, Math.min(maxY, safeCenterY - viewH / 2));
  }
}
...
function setCharacter(id) {
  ...
  if (myCharacterId && !canChangeLimited('character')) return;

  myCharacterId = normalizeCharacterId(id);
  markLimitedChange('character');
...
function npcCollisionRectAt(npc, x = npc.x, y = npc.y) {
  const scale = Number(NPC_CONFIG[npc.kind]?.scale || 1);
  const w = Math.max(20, PLAYER_DRAW_W * scale * 0.36);
  const h = Math.max(22, PLAYER_DRAW_H * scale * 0.36);
  return { x: x - w / 2, y: y - h / 2, w, h };
}

function isNpcBlockedAt(npc, x, y) {
  const rect = npcCollisionRectAt(npc, x, y);

  for (const item of getItems()) {
    if (!item.blocking) continue;
    const blockRect = collisionRect(item);
    if (rectsHit(rect, blockRect)) return true;
  }

  for (const item of randomSceneryTiles) {
    if (!item.blocking) continue;
    const blockRect = sceneryCollisionRect(item);
    if (blockRect && rectsHit(rect, blockRect)) return true;
  }

  for (const item of getFixedEditorMapTiles()) {
    if (!item.blocking || shouldHideFixedMapTileForHome(item)) continue;
    const blockRect = editorMapCollisionRect(item);
    if (blockRect && rectsHit(rect, blockRect)) return true;
  }
...
function moveNpcSafely(npc, nextX, nextY) {
  let moved = false;
  if (!isNpcBlockedAt(npc, nextX, npc.y)) { npc.x = nextX; moved = true; }
  if (!isNpcBlockedAt(npc, npc.x, nextY)) { npc.y = nextY; moved = true; }
  if (!moved) {
    npc.targetX = npc.x;
    npc.targetY = npc.y;
    npc.moving = false;
...
function drawFixedEditorMapTiles() {
  ...
  for (const item of tiles) {
    if (shouldHideFixedMapTileForHome(item)) continue;
    const rect = editorMapItemRect(item);
    if (!rect) continue;
    ...
    if (img && img.complete && img.naturalWidth) {
      ...
      if (summerAlpha > 0.01) {
        ...
        ctx.restore();
        if (isGifSrc(item.src)) placeGifDom('fixed_' + (item.uid || item.tileId), item.src, p.x, p.y, w, h, true, !!item.flipX, !!item.flipY);
      }
    }
...
function drawMiniMap() {
  ...
  const size = 150;
  const pad = 14;
  const x = canvas.clientWidth - size - pad;
  const y = pad;
  const cellSize = size / WORLD_COLS;
  ctx.save();
  ...
  const px = x + (player.x / (WORLD_COLS * CELL)) * size;
  const py = y + (player.y / (WORLD_ROWS * CELL)) * size;
  ctx.fillStyle = '#22c55e';
  ctx.beginPath(); ctx.arc(px, py, 4.5, 0, Math.PI * 2); ctx.fill();
  const home = parseCell(getHomeCellKey());
  if (home) {
    ctx.fillStyle = '#60a5fa';
    ctx.fillRect(x + (home.col - 1) * cellSize + 2, y + (home.row - 1) * cellSize + 2, cellSize - 4, cellSize - 4);
  }
  const target = getCurrentQuestTarget();
  if (target) {
    const tx = x + (target.x / (WORLD_COLS * CELL)) * size;
    const ty = y + (target.y / (WORLD_ROWS * CELL)) * size;
  ...
  ctx.textAlign = 'right';
  ctx.fillText(cellFromWorld(player.x, player.y)?.key || '', x + size - 6, y + size - 6);
  ctx.restore();
}
...
function initUI() {
  ...
  bind('openAdminBtn', 'click', openAdminModal);
  bind('adminCloseBtn', 'click', () => document.getElementById('adminModal')?.classList.add('hidden'));
...
function setHomeCamera() {
  ...
  if (!cell) return;
  if (!isSafeHomeCell(cell.key)) return showToast('لا يمكن اختيار خلية فيها مهمة أو NPC أو عنصر مرتبط بمرحلة');
...
function moveHomeToCurrentCell() {
  ...
  if (!isSafeHomeCell(newCell)) return showToast('لا يمكن اختيار خلية فيها مهمة أو NPC أو عنصر مرتبط بمرحلة');
...
function saveSettingsName() {
  const input = document.getElementById('settingsNameInput');
  if (!canChangeLimited('name')) return;
  displayName = cleanPlayerName(input?.value, 20);
  markLimitedChange('name');
...
function saveProfileData() {
  ...
  writeProfile(window.ref(window.db, 'profiles/' + user.uid), {
    ...
    lastDir: player.dir,
    lastNameChangeAt: Number(lastNameChangeAt || 0),
    lastCharacterChangeAt: Number(lastCharacterChangeAt || 0),
    realEmail: adminRealEmail || '',
    updatedAt: Date.now()
  }).catch(error => {
...
function loadProfileData() {
  ...
  window.get(window.ref(window.db, 'profiles/' + user.uid)).then(snapshot => {
    ...
    lastNameChangeAt = Number(data.lastNameChangeAt || 0);
    lastCharacterChangeAt = Number(data.lastCharacterChangeAt || 0);
    adminRealEmail = String(data.realEmail || data.pendingRealEmail || '').toLowerCase();
...
function signup() {
  ...
  window.createUserWithEmailAndPassword(window.auth, email, pass).then(credential => {
    ...
    closeAuthModal();
    placePlayerAtRandomSafeCell(true);
    saveProfileData();
    savePlayerToFirebase();
    updateAuthUI();
    showCharacterModal(true);
    showSettingsHelpOnce();
    showToast('لتغيير مكانك اضغط على وضع التجول وامشِ بعيدًا، ثم اضغط وضع البناء لتحديد بيتك ومنزلك للبناء');
  }).catch(error => {
...
function publicPlayerPayload() {
  const cell = cellFromWorld(player.x, player.y);
  return {
    uid: currentUser?.uid || window.auth?.currentUser?.uid || '',
    name: cleanPlayerName(displayName || emailToUsername(window.auth?.currentUser?.email) || 'لاعب', 40),
    email: window.auth?.currentUser?.email || currentUserEmail || '',
    x: Math.round(player.x),
    y: Math.round(player.y),
    cell: cell?.key || '',
    character: myCharacterId || 'woman-1',
    houseCell: getHomeCellKey() || '',
    houseName: cleanHouseName(houseProfiles[currentUser?.uid]?.name || document.getElementById('houseNameInput')?.value || '', 30),
    updatedAt: Date.now()
  };
}

function savePublicPlayer() {
  if (!isLoggedIn() || !window.db || !window.ref || !window.update) return;
  const uid = window.auth.currentUser.uid;
  window.update(window.ref(window.db, 'publicPlayers/' + uid), publicPlayerPayload()).catch(() => {});
}

function openAdminModal() {
  if (!isAdminUser()) return showToast('هذه اللوحة للمدير فقط');
  const modal = document.getElementById('adminModal');
  modal?.classList.remove('hidden');
  renderAdminPlayers({});
  if (!window.db || !window.ref || !window.get) return;
  Promise.all([
    window.get(window.ref(window.db, 'publicPlayers')).then(s => s.val() || {}).catch(() => ({})),
    window.get(window.ref(window.db, 'players')).then(s => s.val() || {}).catch(() => ({}))
  ]).then(([publicPlayers, online]) => renderAdminPlayers(publicPlayers, online));
}

function renderAdminPlayers(publicPlayers = {}, online = onlinePlayers || {}) {
  const box = document.getElementById('adminPlayersList');
  if (!box) return;
  const ids = new Set([...Object.keys(publicPlayers || {}), ...Object.keys(online || {})]);
  if (!ids.size) { box.innerHTML = '<p class="smallHint">لا يوجد لاعبين محفوظين بعد.</p>'; return; }
  box.innerHTML = [...ids].map(uid => {
    const data = Object.assign({}, publicPlayers[uid] || {}, online[uid] || {});
    const isOnline = !!online[uid];
    const cell = data.cell || cellFromWorld(Number(data.x || 0), Number(data.y || 0))?.key || '--';
    const name = cleanPlayerName(data.name || 'لاعب', 40);
    const house = cleanHouseName(data.houseName || 'بيت بدون اسم', 30);
    return `<div class="adminPlayerCard" data-uid="${uid}">
      <h3>${name}</h3>
      <small><span class="${isOnline ? 'adminStatusOnline' : 'adminStatusOffline'}">${isOnline ? 'متصل' : 'غير متصل'}</span> | الخلية: ${cell} | البيت: ${house}</small>
      <div class="adminPlayerActions">
        <button type="button" onclick="window.__adminRenamePlayer('${uid}')">تغيير اسم اللاعب</button>
        <button type="button" onclick="window.__adminRenameHouse('${uid}')">تغيير اسم البيت</button>
      </div>
    </div>`;
  }).join('');
}

window.__adminRenamePlayer = uid => {
  if (!isAdminUser()) return;
  const name = cleanPlayerName(prompt('اكتب الاسم الجديد للاعب') || '', 40);
  if (!name || !window.db || !window.ref || !window.update) return;
  Promise.all([
    window.update(window.ref(window.db, 'profiles/' + uid), { displayName: name, adminEditedAt: Date.now() }),
    window.update(window.ref(window.db, 'publicPlayers/' + uid), { name, updatedAt: Date.now() })
  ]).then(() => { showToast('تم تغيير اسم اللاعب'); openAdminModal(); }).catch(error => { console.error(error); showToast('فشل تغيير الاسم'); });
};

window.__adminRenameHouse = uid => {
  if (!isAdminUser()) return;
  const name = cleanHouseName(prompt('اكتب اسم البيت الجديد') || '', 30);
  if (!name || !window.db || !window.ref || !window.update) return;
  Promise.all([
    window.update(window.ref(window.db, 'houseProfiles/' + uid), { name, updatedAt: Date.now() }),
    window.update(window.ref(window.db, 'publicPlayers/' + uid), { houseName: name, updatedAt: Date.now() })
  ]).then(() => { showToast('تم تغيير اسم البيت'); openAdminModal(); }).catch(error => { console.error(error); showToast('فشل تغيير اسم البيت'); });
};

function updateDataLoadProgress() {
  const steps = [
    { ok: typeof fixedEditorMapTiles !== 'undefined', text: 'الخريطة' },
    { ok: typeof NPC_CONFIG !== 'undefined' && typeof MISSION_DEFINITIONS !== 'undefined', text: 'المهام' },
    { ok: !!window.db && !!window.auth, text: 'Firebase' },
    { ok: !isLoggedIn() || gameStateFirebaseReady, text: 'بيانات اللاعب' },
    { ok: !isLoggedIn() || worldNpcsLoaded, text: 'الشخصيات' },
    { ok: true, text: 'الواجهة' }
  ];
  const done = steps.filter(s => s.ok).length;
  const percent = Math.round(done / steps.length * 100);
  const fill = document.getElementById('dataLoadFill');
  const pct = document.getElementById('dataLoadPercent');
  const txt = document.getElementById('dataLoadText');
  const box = document.getElementById('dataLoadBox');
  if (fill) fill.style.width = percent + '%';
  if (pct) pct.textContent = percent + '%';
  const pending = steps.find(s => !s.ok);
  if (txt) txt.textContent = pending ? `جاري تحميل ${pending.text}...` : 'اكتمل تحميل البيانات';
  if (box) box.classList.toggle('done', percent >= 100);
}
