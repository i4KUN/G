...
// الصور تبقى على GitHub، واللعبة لا تحتوي على مجلد All-Pic داخل ZIP.
// jsDelivr أخف عادة من raw.githubusercontent.com ويقلل تعليق تحميل الصور.
export const ASSET_ROOT = 'https://cdn.jsdelivr.net/gh/i4KUN/G@main/';
export const ASSET_FALLBACK_ROOT = 'https://raw.githubusercontent.com/i4KUN/G/main/';
export const assetUrl = path => /^https?:\/\//.test(path) ? path : ASSET_ROOT + path.replace(/^\.\//, '');
export const assetFallbackUrl = path => /^https?:\/\//.test(path) ? path : ASSET_FALLBACK_ROOT + path.replace(/^\.\//, '');
...
const moneyCells = ['B2','D4','F6','H8','J10','L12','N14','P16','R18','T20','C17','E15','G13','I11','K9','M7','O5','Q3','S19','A20','B10','D12','F14','H16','J18'];
export const mapMoney = moneyCells.map((cellKey, i) => {
  const parts = cellKey.match(/^([A-Z]+)(\d+)$/);
  const col = parts[1].charCodeAt(0) - 64;
