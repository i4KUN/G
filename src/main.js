...
function bindSceneUi() {
  ...
  document.getElementById('saveHouseNameBtn')?.addEventListener('click', () => requireScene()?.saveHouseName(document.getElementById('houseNameInput')?.value));
  ...
  document.getElementById('mobileUndoBtn')?.addEventListener('click', () => requireScene()?.undoLast());
...
document.getElementById('loginSubmitBtn')?.addEventListener('click', async event => {
  ...
  try {
    ...
    toast.show(err.message || 'فشل تسجيل الدخول');
  } finally {
    btn.disabled = false;
  }
});
document.getElementById('signupSubmitBtn')?.addEventListener('click', async event => {
  ...
  try {
    ...
    toast.show(err.message || 'فشل إنشاء الحساب');
  } finally {
    btn.disabled = false;
  }
});
